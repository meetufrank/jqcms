...
class Mailbox extends Controller
{
   ...
   public function index($id=0){
       ...
       if($id==0&&empty(input('keyword'))){
        ...
        $count= Db('heademail')->where($map)->count();//获取数据的总数量
        $emaillist = db('heademail')
                ->where($map)
                ->order($order)
                ->paginate($pageSize,$count/*false*/,[//数字10为每页显示的总条数，true为去掉中间的页码部分，false为显示分页的页码
            'type'       => 'page\Pageer',//分页类名
            'var_page' => 'page',//分页变量
            'page'     => $num,//传入跳转值给当前页
           ]);
        ...
        //print_r($emaillist);exit;
         ...
         if($id==0&&!empty(input('keyword'))){
           ...
           $this->assign('selectid',$listarr['data'][0]['header']);
           $grouptitle=db('auth_group')->where(['group_id'=>$listarr['data'][0]['header']])->value('title');
