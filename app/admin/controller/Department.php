...
//设置状态
    ...
    function update(){
        ...
        if(!$only){
            $result['msg'] = lang('Department').lang('name').lang('Occupied');
            $result['code'] = 0;
            return $result;
        }
        $list=$model->update($data);
        if (false !== $list) {
            OprateLogic::getInstance()->insert(lang('Department management').lang('colon').lang('Information changes'),$data['id']);   //存储操作日志
            if($controllerName=='Page'){
    ...
    public function insert(){
        ...
        if ($id !==false) {
            OprateLogic::getInstance()->insert(lang('Department management').lang('colon').lang('Information added'),$id);   //存储操作日志
 ...
 */
  public function reduction(){
      if(empty(input('post.id'))){
        ...
        $model = $this->dao;
        //查询还原后是否有重复的名称
        $content=$model->where(array('id'=>$id))->find();
        $where=[
            'id'=>['neq',$id]
        ];
        $only=$this->isonly($content,$where);
        if(!$only){
            $result['msg'] = lang('Department').lang('name').lang('Occupied');
            $result['code'] = 0;
            return $result;
        ...
        OprateLogic::getInstance()->insert(lang('Department management').lang('colon').lang('Information restoration'),$id);   //存储操作日志
    ...
    public function listDel(){
        ...
        OprateLogic::getInstance()->insert(lang('Department management').lang('colon').lang('Transferring information to recycle bin'),$id);   //存储操作日志
    ...
    public function delAll(){
        ...
        OprateLogic::getInstance()->insert(lang('Department management').lang('colon').lang('Bulk transfer to recycle bin'), implode(',',input('param.ids/a')));   //存储操作日志
        $result['code'] = 1;
        ...
        OprateLogic::getInstance()->insert(lang('Department management').lang('colon').lang('Delete information completely'),$id);   //存储操作日志
    ...
    public function removeAll(){
        ...
        OprateLogic::getInstance()->insert(lang('Department management').lang('colon').lang('Batch delete information completely'), implode(',',input('param.ids/a')));   //存储操作日志
        $result['code'] = 1;
     ...
     */
       public function reductionAll(){
        ...
        $content=$model->where($map)->select();
        ...
        //需要查询这组待还原数据是否有重复的名称
        if (count($namearr) != count(array_unique($namearr))){
            $result['msg'] = lang('Restore data that violates the rules');
            $result['code'] = 0;
            return $result;
        }
        ...
        $model->where($map)->update($data);
        OprateLogic::getInstance()->insert(lang('Department management').lang('colon').lang('Information batch restore'), implode(',',input('param.ids/a')));   //存储操作日志
        $result['code'] = 1;
